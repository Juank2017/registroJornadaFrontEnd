...
export class SedesService {
    ...
    return this._http.get(url).pipe(
      map((resp: any) => {
        const empresa: Empresa = new Empresa(
          resp.sede.empresa.id,
          resp.sede.empresa.nombre,
          resp.sede.empresa.cif
        );
  ...
  getSedeByEmpresaId(idEmpresa: string) {
    const url =
      URL_SERVICIOS + 'sedes/empresa/' + idEmpresa + '?token=' + this.token;
  ...
  deleteSede(sede: Sede) {
    const url =
      URL_SERVICIOS + 'sedes/delete/' + sede.id + '?token=' + this.token;
